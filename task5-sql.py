...
result = spark.sql('SELECT vehicle_make , COUNT(*) C1 FROM park_vio GROUP BY vehicle_make ORDER BY C1 DESC limit 1')

result.select(format_string('%s\t%d', result.vehicle_make, result.C1)).write.save("task5-sql.out", format = "text")
